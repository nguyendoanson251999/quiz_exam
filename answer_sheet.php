...
<body>
	...
	<div class="container-fluid admin">
		<div class="col-md-12 alert alert-primary"><?php echo $quiz['title'] ?> | <?php echo $quiz['qpoints'] .' Points Each Question' ?>
		</div>
		<br>
		<div class="card">
			<div class="card-body">
				<form action="" id="answer-sheet">
					...
					<?php
					$question = $conn->query("SELECT * FROM questions where qid = '".$quiz['id']."' order by RAND() LIMIT 60 ");
					$i = 1 ;
					...
					<li class="q-field list-group-item">
						...
						<?php while($orow = $opt->fetch_assoc()){ ?>
							...
							<li class="answer list-group-item">
								...
								<label><input type="radio" name="option_id[<?php echo $row['id']; $qs = $row['question']; ?>]" value="<?php echo $orow['id'] ?>"> <?php echo $orow['option_txt'] ?></label>
							</li>
						<?php } ?>
